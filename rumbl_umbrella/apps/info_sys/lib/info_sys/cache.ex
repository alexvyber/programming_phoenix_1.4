defmodule InfoSys.Cache do
  ...
  @clean_interval :timer.seconds(60)
  ...
  def init(opts) do
    state = %{
      interval: opts[:clean_interval] || @clean_interval,
      timer: nil,
      table: new_table(opts[:name])
    }

    {:ok, schedule_clean(state)}
  end

  def handle_info(:clean, state) do
    :ets.delete_all_objects(state.table)
    {:noreply, schedule_clean(state)}
  end

  defp schedule_clean(state) do
    %{state | timer: Process.send_after(self(), :clean, state.interval)}
  end
